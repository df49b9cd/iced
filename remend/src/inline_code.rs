...
fn handle_inline_triple_backticks(text: &str) -> Option<Cow<'_, str>> {
    // Must be single-line and match: starts with ```, no newlines.
    if text.contains('\n') {
        return None;
    }
    if !text.starts_with("```") {
    ...
    // Check it looks like inline triple backtick usage.
    let rest = &text[3..];
    if rest.contains('`') && !rest.ends_with("```") {
        // Already has some backticks but not a complete closing ```.
