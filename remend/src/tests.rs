...
use super::{remend, LinkMode, RemendOptions};
...
#[test]
fn bold_complete() {
    assert_eq!(r("Text with **bold text**").as_ref(), "Text with **bold text**");
}

#[test]
fn bold_multiple_complete() {
    assert_eq!(r("**bold1** and **bold2**").as_ref(), "**bold1** and **bold2**");
}

#[test]
fn bold_odd_markers() {
    assert_eq!(r("**first** and **second").as_ref(), "**first** and **second**");
}

#[test]
fn bold_partial_boundary() {
    assert_eq!(r("Here is some **bold tex").as_ref(), "Here is some **bold tex**");
}
...
fn italic_double_underscore_odd() {
    assert_eq!(r("__first__ and __second").as_ref(), "__first__ and __second__");
}
...
fn italic_asterisk_complete() {
    assert_eq!(r("Text with *italic text*").as_ref(), "Text with *italic text*");
}
...
fn italic_underscore_complete() {
    assert_eq!(r("Text with _italic text_").as_ref(), "Text with _italic text_");
}
...
fn bold_italic_incomplete() {
    assert_eq!(r("Text with ***bold-italic").as_ref(), "Text with ***bold-italic***");
}
...
fn strikethrough_complete() {
    assert_eq!(r("~~strikethrough text~~").as_ref(), "~~strikethrough text~~");
}
...
fn katex_inline_enabled_escaped() {
    let opts = RemendOptions::default().inline_katex(true);
    assert_eq!(
        remend("Price is \\$100", &opts).as_ref(),
        "Price is \\$100"
    );
}
...
fn katex_dollar_in_inline_code() {
    assert_eq!(
        r("Streamdown uses double dollar signs (`$$`) to delimit mathematical expressions.").as_ref(),
...
fn link_text_only_image_removed() {
    let opts = RemendOptions::default().link_mode(LinkMode::TextOnly);
    assert_eq!(
        remend("Text ![incomplete image", &opts).as_ref(),
        "Text"
    );
}
...
fn streaming_nested_formatting() {
    ...
    assert_eq!(r("This is **bold with *ital").as_ref(), "This is **bold with *ital*");
}
