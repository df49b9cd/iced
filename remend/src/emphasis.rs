...
use super::utils::{
    cow_append, is_empty_or_markers, is_horizontal_rule, is_inside_code_block, is_list_marker_line,
    is_within_complete_inline_code, is_within_html_tag, is_within_link_or_image_url,
...
fn should_skip_asterisk(text: &str, index: usize, prev: u8, next: u8, has_dollar: bool) -> bool {
    ...
    if index > 0 && index + 1 < text.len() {
        ...
        if let (Some(pc), Some(nc)) = (prev_char, next_char)
            && is_word_char(pc) && is_word_char(nc)
        {
            return true;
        }
    }
...
pub fn count_triple_asterisks(text: &str) -> usize {
    ...
    for i in 0..len {
            ...
            && bytes[i + 2] == b'`'
        {
            ...
            consecutive = 0;
            in_code_block = !in_code_block;
            // Note: we don't skip i+2 here since we're just counting stars,
            // but the code block flag is set correctly.
            continue;
        }
        if in_code_block {
            continue;
...
pub fn handle_double_underscore(text: &str) -> Cow<'_, str> {
    ...
    if let Some(pos) = find_half_complete_underscore(text)
        ...
        && !is_within_complete_inline_code(text, pos)
    {
        let pairs = count_double_underscores(text);
        if pairs % 2 == 1 {
...
pub fn handle_italic_asterisk(text: &str) -> Cow<'_, str> {
    ...
    if is_inside_code_block(text, first_idx)
        || is_within_complete_inline_code(text, first_idx)
    {
        return Cow::Borrowed(text);
    }
...
mod tests {
    ...
    fn double_underscore_half_complete() {
        assert_eq!(
            handle_double_underscore("__italic_").as_ref(),
            "__italic__"
        );
    }
