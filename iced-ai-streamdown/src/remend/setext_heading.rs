...
pub fn handle(text: &str) -> Cow<'_, str> {
    ...
    // Check for 1-2 dashes (not 3+, which is a valid horizontal rule / setext heading).
    let is_dash = trimmed == "-" || trimmed == "--";
    let is_equals = trimmed == "=" || trimmed == "==";

    if !is_dash && !is_equals {
        return Cow::Borrowed(text);
    }

    // Don't modify if last line has trailing space (already breaks the pattern).
    if is_dash && last_line.ends_with(' ') {
        return Cow::Borrowed(text);
    }
    if is_equals && last_line.ends_with(' ') {
        return Cow::Borrowed(text);
    }
