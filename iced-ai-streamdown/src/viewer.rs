...
pub fn animate_spans_no_caret(
    ...
    animate_spans(base_spans, animation, global_word_offset, now, settings, false)
}

/// Splits text spans into per-word sub-spans with animation opacity applied.
///
// TODO: Cache the word-split structure when content changes. On each frame,
// only update color/opacity — word boundaries don't change between frames.
fn animate_spans(
    base_spans: &[text::Span<'static, markdown::Uri>],
    animation: &AnimationState,
    global_word_offset: usize,
    ...
    let default_color = settings.text_color;
    ...
    for span in base_spans {
        ...
        while chars.peek().is_some() {
            ...
            if ws_end > ws_start {
                ...
                animated.push(make_animated_span(span, ws.to_owned(), opacity, default_color));
            }
            ...
            if word_end > word_start {
                ...
                animated.push(make_animated_span(span, word.to_owned(), opacity, default_color));
                word_idx += 1;
            }
        }
    }
    ...
    if show_caret {
        if let Some(caret_kind) = settings.caret {
            let caret_color = settings.caret_color.unwrap_or(default_color);
...
fn make_animated_span(
    ...
    let color = base.color.unwrap_or(default_color);
